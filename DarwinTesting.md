# Darwin Testing Strategy & Execution Guide
...
Current source-contract status: the focused source-contract lanes pass with zero skipped tests. Optional MinIO smoke tests are skipped only when `DARWIN_RUN_MINIO_SMOKE=true` and the required local MinIO environment variables are not configured, which is expected for normal CI and machines without local MinIO.
...
- Mobile launch-readiness source contracts: Android cleartext traffic disabled, committed mobile Google API keys absent, unsafe certificate trust guarded outside `DEBUG`, and English/German resource key parity.
- Member-commerce invoice artifact routes and service calls, including archive document plus structured JSON/XML source-model downloads. These tests guard route/service behavior only; they do not claim e-invoice compliance.

#### Current progress snapshot (2026-05-20)

- [x] `MemberCommerceServiceTests`: validation checks for paging and IDs across order/invoice read/detail/document/payment paths.
- [x] `MemberCommerceServiceTests`: positive route coverage for invoice/order/detail/document/payment-intent calls.
- [x] `MemberCommerceServiceTests`: cache behavior for member commerce (`GetFresh` short-circuit and `GetUsable` fallback on API failures).
- [x] `MemberCommerceServiceTests`: network-exception mapping for GET/POST/string-download flows using `MobileErrorMessages.NetworkFailure`.
- [x] `MemberCommerceServiceTests`: added remaining payment-oriented failure-path coverage (`GetOrder`, `GetInvoice`, create-payment-intent calls, and invoice/order/download flows), plus canonical route assertions for `GetMyOrders`, `GetInvoice`, and invoice document.
- [x] `LoyaltyServiceTests` (new): initial loyalty coverage for consumer-facing and business scan-flow validation/routing/network-failure behavior.
- [x] `LoyaltyServiceTests` (new): payment-related loyalty dependencies (`GetCurrentBusinessSubscriptionStatus`, `GetBillingPlans`, `SetCancelAtPeriodEnd`, and `CreateSubscriptionCheckoutIntent`) plus route/error mapping.
- [x] `LoyaltyServiceTests` (new): expanded mobile-loyalty coverage for campaign mutation call paths, including `UpdateBusinessCampaignAsync` and `SetBusinessCampaignActivationAsync` network-failure mapping, plus `FakeApiClient.PutNoContentAsync` support.
- [x] `LoyaltyServiceTests` (new): expanded network-failure coverage for additional loyalty surfaces (`GetAccountSummaryAsync`, `GetBusinessDashboardAsync`, timeline page, promotion tracking, reward-tier/campaign mutations, business reward configuration, and loyalty promotions).
- [x] `LoyaltyServiceTests` (new): added network-failure coverage for core loyalty flows and billing dependencies (`JoinLoyaltyAsync`, `GetMyAccountsAsync`, `GetMyOverviewAsync`, `ProcessScanSessionForBusinessAsync`, `ConfirmAccrualAsync`, `GetCurrentBusinessSubscriptionStatusAsync`, `SetCancelAtPeriodEndAsync`, `GetBillingPlansAsync`).
- [x] `LoyaltyServiceTests` (new): added missing validation coverage for `GetMyBusinessesAsync` when `page` is invalid.
- [x] `Loyalty`-aware consumer auth email-flow coverage added: `AuthServiceInvitationTests` validates `ConfirmEmail` and failure behavior.
- [x] `AuthServiceInvitationTests`: added failure-path coverage for `RequestEmailConfirmationAsync`.
- [x] `AuthServiceInvitationTests`: added exception-propagation coverage for email-confirmation calls (`RequestEmailConfirmationAsync`, `ConfirmEmailAsync`) to match current transport behavior.
- [x] `AuthServiceInvitationTests`: added mobile auth route/behavior coverage for `LoginAsync`, `RegisterAsync`, `ChangePasswordAsync` (including unauthorized-refresh retry path), `RequestPasswordResetAsync`, `ResetPasswordAsync`, and `LogoutAllAsync`.
- [x] `AuthServiceInvitationTests`: added session-lifecycle coverage for `EnsureAuthenticatedSessionAsync` and additional refresh session cleanup edge-case (`TryRefreshAsync` when refresh token is missing).
- [x] `AuthServiceInvitationTests`: added missing auth-service outcomes for register/reset/request-password-reset failure, exception behavior, and cached bootstrap behavior in `EnsureAuthenticatedSessionAsync`.
...
- [done] Add external-command smoke evidence for legal-validation failure propagation to protect against non-compliant artifacts even when process exit code is zero:
  - `EInvoiceExternalCommandSmoke_Should_Fail_When_ValidationReport_Reports_FailedLegalValidation` in `SecurityAndPerformanceContractsAndPackagingSourceTests` confirms that `--validation-report` negative results fail the smoke.
...
- Infrastructure external-command coverage now also proves the command adapter passes the expected `--format`, `--validation-profile`, and `--validation-report` values to deployment-approved wrappers:
...
New unit tests added on 2026-05-18 (null-DB-RowVersion coverage session):

- `dotnet test tests/Darwin.Tests.Unit/Darwin.Tests.Unit.csproj --filter "FullyQualifiedName~InventoryManagementHandlerTests" --no-build`
  - Extended with 10 new tests: `UpdateWarehouse_Should_ThrowConcurrency_WhenDbRowVersionIsNull`, `UpdateSupplier_Should_ThrowValidation_WhenRowVersionIsEmpty`, `UpdateSupplier_Should_ThrowConcurrency_WhenDbRowVersionIsNull`, `UpdateStockLevel_Should_ThrowValidation_WhenRowVersionIsEmpty`, `UpdateStockLevel_Should_ThrowConcurrency_WhenDbRowVersionIsNull`, `UpdateStockTransfer_Should_ThrowValidation_WhenRowVersionIsEmpty`, `UpdateStockTransfer_Should_ThrowConcurrency_WhenDbRowVersionIsNull`, `UpdatePurchaseOrder_Should_ThrowValidation_WhenRowVersionIsEmpty`, `UpdatePurchaseOrder_Should_ThrowConcurrency_WhenDbRowVersionIsNull`, `PurchaseOrderLifecycle_Should_ReturnFail_WhenRowVersionMismatches` - together these complete the full Inventory RowVersion coverage matrix.
- `dotnet test tests/Darwin.Tests.Unit/Darwin.Tests.Unit.csproj --filter "FullyQualifiedName~CrmRowVersionCoverageTests" --no-build`
  - 9 passed - new file covers: `UpdateCustomer_Should_ThrowConcurrency_WhenDbRowVersionIsNull`, `UpdateLead_Should_ThrowConcurrency_WhenDbRowVersionIsNull`, `UpdateOpportunity_Should_ThrowConcurrency_WhenDbRowVersionIsNull`, `UpdateCustomerSegment_Should_ThrowConcurrency_WhenDbRowVersionIsNull`, `TransitionInvoiceStatus_Should_ThrowValidation_WhenRowVersionIsEmpty`, `TransitionInvoiceStatus_Should_ThrowConcurrency_WhenDbRowVersionIsNull`, `UpdateInvoice_Should_ThrowValidation_WhenRowVersionIsEmpty`, `UpdateInvoice_Should_ThrowConcurrency_WhenDbRowVersionIsNull`, `CreateInvoiceRefund_Should_ThrowValidation_WhenRowVersionIsEmpty`.
- `dotnet test tests/Darwin.Tests.Unit/Darwin.Tests.Unit.csproj --filter "FullyQualifiedName~LoyaltyRowVersionCoverageTests" --no-build`
  - 16 passed - new file covers: `ActivateLoyaltyAccount_Should_Succeed_WhenNullRowVersionProvided`, `ActivateLoyaltyAccount_Should_Fail_WhenDbRowVersionIsNullAndClientVersionProvided`, `SuspendLoyaltyAccount_Should_Succeed_WhenNullRowVersionProvided`, `SuspendLoyaltyAccount_Should_Fail_WhenDbRowVersionIsNullAndClientVersionProvided`, `AdjustLoyaltyPoints_Should_Succeed_WhenNullRowVersionProvided`, `AdjustLoyaltyPoints_Should_Fail_WhenDbRowVersionIsNullAndClientVersionProvided`, `UpdateLoyaltyProgram_Should_Succeed_WhenNullRowVersionProvided`, `UpdateLoyaltyProgram_Should_ThrowValidation_WhenDbRowVersionIsNullAndClientVersionProvided`, `SoftDeleteLoyaltyProgram_Should_Succeed_WhenNullRowVersionProvided`, `SoftDeleteLoyaltyProgram_Should_Fail_WhenDbRowVersionIsNullAndClientVersionProvided`, `UpdateLoyaltyRewardTier_Should_Succeed_WhenNullRowVersionProvided`, `UpdateLoyaltyRewardTier_Should_ThrowValidation_WhenDbRowVersionIsNullAndClientVersionProvided`, `SoftDeleteLoyaltyRewardTier_Should_Succeed_WhenNullRowVersionProvided`, `SoftDeleteLoyaltyRewardTier_Should_Fail_WhenDbRowVersionIsNullAndClientVersionProvided`.
- `dotnet test tests/Darwin.Tests.Unit/Darwin.Tests.Unit.csproj --filter "FullyQualifiedName~OrderRowVersionCoverageTests" --no-build`
  - 3 passed - new file covers: `ResolveShipmentCarrierException_Should_Fail_WhenDbRowVersionIsNull`, `UpdateShipmentProviderOperation_Should_Fail_WhenDbRowVersionIsNull`, `UpdateOrderStatus_Should_Throw_WhenDbRowVersionIsNull`.
...
- [done] Add provider-backed refund tests proving `AddRefundHandler` calls the Stripe provider only for Stripe payments with a provider reference, stores provider refund id/status/failure details, marks the refund failed when the provider client is null or returns a failure, and skips provider calls for non-Stripe or reference-free payments. (6 tests added to `OrderBillingHandlerTests.cs`)
- [done] Add webhook reconciliation tests for `refund.created`, `refund.updated`, and `charge.refunded` covering: new refund record creation with succeeded/failed status, existing refund update on `refund.updated`, full-refund `charge.refunded` marking payment and order Refunded, partial `charge.refunded` marking order PartiallyRefunded, and silent skip when no matching payment is found. (6 tests added to `ProcessStripeWebhookHandlerTests.cs`)
- [done] Add smoke-script tests for `scripts/smoke-stripe-testmode.ps1 -CheckBusinessSubscriptionCheckout` covering source-contract assertions (switch present, function declared, no printed provider URL/references), dry-run readiness assertion extended to include `Add -CheckBusinessSubscriptionCheckout`, new tests for blocking when checkout prerequisites are missing, and dry-run ready with all checkout prerequisites. (`ProviderSmokeScripts_Should_StayGuardedAndAvoidSecretOutput` extended; `StripeSubscriptionCheckoutSmoke_Should_BlockWhenCheckoutPrerequisitesMissing` and `StripeSubscriptionCheckoutSmoke_Should_ReportReadyDryRunWithCheckoutPrerequisites` added to `SecurityAndPerformanceContractsAndPackagingSourceTests`)
- External Stripe provider smoke after webhook forwarding is configured: storefront checkout payment, verified webhook finalization, business subscription webhook finalization, WebAdmin provider-backed refund reconciliation, and real test-mode `charge.dispute.created` follow-up processing passed through `https://api.loyan.de` on 2026-05-24 with `ProviderCallbackWorker` enabled. Remaining external coverage is approved live-mode smoke and operational monitoring/alerting before production traffic.
- The focused billing update/webhook lane was rechecked after the dispute smoke work. `BillingUpdateHandlersTests` and `ProcessStripeWebhookHandlerTests` now pass together with `45` passed and `0` skipped; the RowVersion/concurrency test harness safely simulates legacy null database row versions without weakening production RowVersion checks.
...
- [done] Extend object-storage smoke-script dry-run branch tests for provider-specific prerequisites and secure readiness paths (`ObjectStorageSmoke_Should_*` in `SecurityAndPerformanceContractsAndPackagingSourceTests` now covers unsupported provider blocking, S3 endpoint/region requirement enforcement, and Azure managed-identity readiness without connection string). External operator-run evidence for target MinIO/S3-compatible deployment still remains a go-live activity.
- [done] Extend DHL smoke-script source-contract coverage for runtime-pipeline and return-flow safety rails (`ProviderSmokeScripts_Should_StayGuardedAndAvoidSecretOutput` now asserts `-RequireRuntimePipeline` readiness/blocking contracts, required worker/storage confirmations, and guarded `-IncludeReturn` path wiring in `smoke-dhl-live.ps1`). Operator-run live-smoke evidence for shipment creation, label retrieval/storage through the configured storage profile, tracking reference persistence, callback processing, failed/stuck operation retry, and carrier exception visibility still remains a go-live activity.
- [done] Add Brevo production-readiness preflight coverage for sandbox send prerequisites, controlled-inbox prerequisites, transactional webhook subscription confirmation, callback inbox persistence, and worker enablement checks. (`BrevoReadinessSmoke_Should_BlockWhenDeliveryPipelinePrerequisitesAreMissing` and `BrevoReadinessSmoke_Should_ReportDeliveryPipelineReadinessInDryRun` added to `SecurityAndPerformanceContractsAndPackagingSourceTests`.) This is internal/source-contract coverage; real Brevo DNS, delivery, and webhook validation still requires provider credentials and account-side confirmation.
- [done] Recheck focused Brevo/communication lanes after readiness cleanup on 2026-05-24: `dotnet test tests/Darwin.WebApi.Tests/Darwin.WebApi.Tests.csproj --filter "FullyQualifiedName~Brevo|FullyQualifiedName~EmailDispatchOperationBackgroundService|FullyQualifiedName~ProviderCallback" --no-restore /p:UseSharedCompilation=false` passed with `308` passed and `0` skipped, and `dotnet test tests/Darwin.Tests.Unit/Darwin.Tests.Unit.csproj --filter "FullyQualifiedName~Brevo|FullyQualifiedName~Communication|FullyQualifiedName~RetryEmailDispatch|FullyQualifiedName~ProviderCallbackInbox" --no-restore /p:UseSharedCompilation=false` passed with `232` passed and `0` skipped.
- [done] Structured invoice model mapping and download flow follow-through is now implemented after tooling decision:
  - `GenerateInvoiceEInvoiceArtifactHandlerTests` now includes UnsupportedFormat short-circuit coverage, storage wiring verification for successful generation, and explicit XRechnung path coverage.
  - `WebAdminSecuritySmokeTests` now includes `DownloadInvoiceEInvoiceArtifact` coverage for missing source snapshot, successful ready-generator artifact response, unsupported format fallback, and XRechnung export response format.
- [done] WebAdmin operations testing slice progressed in this pass:
  - `WebAdminSecuritySmokeTests` now validates case-insensitive and whitespace-trimmed operational actions for billing webhook delivery, payment dispute review, provider callbacks, CRM lead/opportunity lifecycle, and shipment provider operations.
  - Post-save concurrency is now asserted for supplier edit and for billing webhook delivery, payment dispute review, provider callback, and shipment provider operation actions; responses still include CSP `form-action 'self'` and concurrency messaging.
- External-command smoke evidence was extended to prove configurable validation-profile handoff end-to-end:
  - `ExternalCommandEInvoiceGenerationServiceTests` now verifies `ValidationProfile` is passed as `--validation-profile` and preserved in the artifact metadata.
  - `SecurityAndPerformanceContractsAndPackagingSourceTests.EInvoiceExternalCommandSmoke_Should_Pass_CustomValidationProfile_ToConfiguredAdapter` verifies `DARWIN_EINVOICE_VALIDATION_PROFILE` is consumed by the command path and surfaced in smoke output.
  - `EInvoiceExternalCommandSmoke_Should_Fail_When_ValidationReport_Reports_FailedLegalValidation` now extends smoke evidence to ensure failed validation reports are surfaced as go-live blockers.
  - Remaining gap: production validation-signed fixtures and full release evidence from selected-tool legal report/smoke still remain the go-live blocker for claiming full e-invoice compliance.
...
- [done] Add unit tests for Billing management query handlers covering `GetPaymentsPageHandler`, `GetPaymentOpsSummaryHandler`, and `GetPaymentForEditHandler` (20 tests in new `BillingManagementQueryHandlerTests.cs`).
- [done] Add unit tests for Billing refund query handlers covering `GetRefundsPageHandler` and `GetRefundOpsSummaryHandler` (18 tests in new `BillingRefundQueryHandlerTests.cs`).
- [done] Add unit tests for financial query handlers covering `GetFinancialAccountsPageHandler`, `GetFinancialAccountForEditHandler`, `GetExpensesPageHandler`, `GetExpenseForEditHandler`, `GetJournalEntriesPageHandler`, and `GetJournalEntryForEditHandler` (35 tests in new `BillingFinancialQueryHandlerTests.cs`).
- [done] Add Billing management command handler tests covering `BillingStatusTransitionPolicy` allowed/forbidden/same-status transitions, `CreatePaymentHandler` validation/persistence/currency normalization, `UpdatePaymentHandler` not-found/empty-RowVersion/stale-RowVersion/forbidden-transition/success, `CreateFinancialAccountHandler`/`UpdateFinancialAccountHandler`/`CreateExpenseHandler`/`UpdateExpenseHandler`/`CreateJournalEntryHandler`/`UpdateJournalEntryHandler` coverage with RowVersion guards and validation (52 tests in new `BillingManagementCommandHandlerTests.cs`).
- [done] Add order billing boundary tests proving payments cannot be created as Refunded/Voided, PaidAtUtc is set for Captured payments, early-stage orders advance to Paid on Captured payments, currency mismatch rejection, and refunds are only allowed for Captured/Completed payments (7 new tests added to `OrderBillingHandlerTests.cs`).
- [done] Add UpdateBusiness RowVersion coverage proving not-found, empty RowVersion, stale RowVersion errors, plus field trimming and whitespace-only null normalization (5 tests added to `BusinessCreateUpdateHandlersTests.cs`).
- [done] Add Business lifecycle transition boundary tests proving approval only applies from PendingApproval, suspension only from Approved, reactivation only from Suspended, plus RowVersion guards for each lifecycle action (14 tests added to `BusinessLifecycleHandlersTests.cs`).
- [done] Add Business media handler tests covering `UpdateBusinessMediaHandler` not-found, empty/stale RowVersion, persistence with trimming, whitespace-only caption nullification, and `DeleteBusinessMediaHandler` empty-Id/empty-RowVersion/not-found failure paths, successful delete (dto and id overloads), and silent no-op on missing id (12 tests in new `BusinessMediaHandlerTests.cs`).
- [done] Add CRM pipeline lifecycle handler tests covering `UpdateLeadLifecycleHandler` (empty Id, empty RowVersion, not-found, stale RowVersion, unsupported action, valid transitions, converted lead blocking) and `UpdateOpportunityLifecycleHandler` (empty Id, empty RowVersion, not-found, stale RowVersion, ClosedWon/ClosedLost advance rejection, stage transitions including ADVANCE/CLOSEWON/CLOSELOST/REOPEN, close-date defaulting and preservation) (28 tests in new `CrmPipelineLifecycleHandlerTests.cs`).
...
- [done] Add provider-selection smoke coverage proving `Persistence:Provider=PostgreSql` resolves Npgsql and `Persistence:Provider=SqlServer` resolves SQL Server. (`ServiceCollectionExtensionsConfiguredPersistenceTests` in `tests/Darwin.Infrastructure.Tests/Persistence`)
- [done] Add PostgreSQL migration/seed verification against a fresh PostgreSQL database. (`FreshPostgreSqlDatabase_Should_ApplyMigrations_AndSeedExpectedRows` in `tests/Darwin.Tests.Integration/Persistence`)
- [done] Add provider-specific migration coverage proving active Shipping Methods enforce unique `Carrier` + `Service` pairs through SQL Server filtered indexes and PostgreSQL partial indexes, matching the Application/WebAdmin validation contract.
(`ShippingMethodCarrierServiceIndexProviderSpecificTests` in `tests/Darwin.Infrastructure.Tests/Persistence`).
- [done] Add a guard that shared EF mappings do not introduce SQL Server-only column types such as `uniqueidentifier` or `nvarchar(max)` in shared persistence mappings.
(`SharedEfMappingProviderAgnosticColumnTypesTests` in `tests/Darwin.Infrastructure.Tests/Persistence`).
- [done] Add filtered-index SQL coverage for PostgreSQL so SQL Server filters are normalized before migrations are applied.
(`PostgreSqlFilteredIndexMigrationSqlTests` in `tests/Darwin.Infrastructure.Tests/Persistence`).
- [done] Add concurrency coverage for `RowVersion`: SQL Server native rowversion and PostgreSQL client-managed bytea concurrency.
(`RowVersionProviderConcurrencyModelTests` in `tests/Darwin.Infrastructure.Tests/Persistence`).
- [done] Add schema-placement coverage proving no application tables are created in PostgreSQL `public` or SQL Server `dbo`.
(`SchemaPlacementModelTests` in `tests/Darwin.Infrastructure.Tests/Persistence`).
- [done] Add migration-script guard coverage that generates idempotent provider scripts and fails if PostgreSQL introduces unwanted `dbo`/application tables in `public`, or if the latest SQL Server/PostgreSQL model snapshots contain unqualified application `ToTable(...)` mappings.
(`MigrationScriptSchemaGuardTests` in `tests/Darwin.Infrastructure.Tests/Persistence`).
- [done] Add SQL Server fresh-bootstrap coverage proving the full SQL Server migration lane applies to an empty database and leaves no application tables in `dbo`.
(`FreshSqlServerDatabase_Should_ApplyMigrations_AndSeedExpectedRows_WithoutApplyingApplicationTablesToDbo` in `tests/Darwin.Tests.Integration/Persistence/PostgreSqlMigrationAndSeedTests.cs`).
- [done] Add PostgreSQL extension/index coverage proving `pg_trgm`, the expected JSON/text-JSON GIN indexes, and the 88+ direct `IX_PG_*_Like_Trgm` search indexes exist after migration.
(`GenerateCreateScript_ForPostgreSql_Should_ContainExpectedSearchExtensionsAndIndexes` in `tests/Darwin.Infrastructure.Tests/Persistence/PostgreSqlSearchIndexCoverageTests.cs`).
- [done] Add PostgreSQL `citext` coverage proving identifier uniqueness and equality behave case-insensitively for login identifiers, role/permission keys, slugs, SKUs, billing plan codes, promotion codes, and tax category names.
(`PostgreSqlProviderSpecificColumnTypeTests` and `PostgreSqlSearchIndexCoverageTests` in `tests/Darwin.Infrastructure.Tests/Persistence`).
- [done] Add provider-neutral canonical lookup coverage proving media asset roles and shipping DHL carrier markers are normalized on write and queried with direct equality, and promotion code/tax category equality relies on canonical/case-insensitive storage rather than query-side `ToLower()` or `ToUpper()`.
(`CreateMediaAsset_Should_Normalize_Known_Roles_For_Canonical_Lookup`, `GetMediaAssetsPage_Should_Filter_EditorAssets_By_Canonicalized_Role`, `CreateShippingMethod_Should_Normalize_Dhl_Carrier_To_Canonical_Value`, `GetShippingMethodsPage_Should_Filter_Dhl_By_Canonicalized_Carrier`, `CreatePromotion_Should_Normalize_Code_To_Canonical_Form`, `CreatePromotion_Should_Reject_Duplicate_Code_After_Normalization`, `UpdatePromotion_Should_Normalize_Code_To_Canonical_Form`, `UpdatePromotion_Should_Reject_Code_Conflicts_For_Canonicalized_Input`, `CreateTaxCategory_Should_Normalize_Name_For_Canonical_Lookup`, `UpdateTaxCategory_Should_Normalize_Name_For_Canonical_Lookup`)
- [done] Add PostgreSQL `jsonb` coverage proving the current 21 selected operational/configuration JSON columns are created as `jsonb`, and the current 14 `IX_PG_*_JsonbGin` indexes exist.
(`PostgreSqlProviderSpecificColumnTypeTests` and `PostgreSqlSearchIndexCoverageTests` in `tests/Darwin.Infrastructure.Tests/Persistence`).
- [done] Add PostgreSQL trigram coverage proving text-backed JSON search columns keep their `IX_PG_*_Trgm` indexes for event-log properties, provider callback payloads, and business admin text overrides.
(`GenerateCreateScript_ForPostgreSql_Should_ContainTextBackedJsonTrgmIndexes` in `tests/Darwin.Infrastructure.Tests/Persistence/PostgreSqlSearchIndexCoverageTests.cs`)
- [done] Add PostgreSQL JSON validity coverage proving the current 11 `CK_PG_*_ValidJson` constraints exist for text-backed JSON columns and reject invalid JSON on new writes.
(`FreshPostgreSqlDatabase_Should_RejectInvalidJsonWrites_ForAllJsonConstrainedColumns` in `tests/Darwin.Tests.Integration/Persistence/PostgreSqlMigrationAndSeedTests.cs`;
`GenerateCreateScript_ForPostgreSql_Should_ContainJsonValidityConstraints` in `tests/Darwin.Infrastructure.Tests/Persistence/PostgreSqlSearchIndexCoverageTests.cs`)
- [done] Add provider-neutral search coverage for Identity user and permission list paths, proving escaped substring search terms and uppercase/lowercase variants return the same relevant rows on PostgreSQL and SQL Server. (`GetUsersPage_Should_HandleEscapedSubstringAndCaseVariants_OnEmailSearch`, `GetPermissionsPage_Should_HandleEscapedSubstringAndCaseVariants_OnKeySearch` in `tests/Darwin.Tests.Integration/Identity/IdentitySearchProviderNeutralIntegrationTests.cs`)
- [done] Add provider-neutral search coverage for Identity user, permission, and mobile-device list paths, proving escaped substring search terms and uppercase/lowercase variants return the same relevant rows on PostgreSQL and SQL Server. (`GetUsersPage_Should_HandleEscapedSubstringAndCaseVariants_OnEmailSearch`, `GetPermissionsPage_Should_HandleEscapedSubstringAndCaseVariants_OnKeySearch`, `GetMobileDevicesPage_Should_HandleEscapedSubstringAndCaseVariants_OnUserEmailSearch` in `tests/Darwin.Tests.Integration/Identity/IdentitySearchProviderNeutralIntegrationTests.cs`)
- [done] Add provider-neutral search coverage for CRM operator, inventory operator (`GetWarehousesPage_Should_HandleEscapedSubstringAndCaseVariants_OnNameSearch`, `GetSuppliersPage_Should_HandleEscapedSubstringAndCaseVariants_OnEmailSearch`, `GetStockLevelsPage_Should_HandleEscapedSubstringAndCaseVariants_OnSkuSearch`, `GetStockTransfersPage_Should_HandleEscapedSubstringAndCaseVariants_OnWarehouseNameSearch`, `GetPurchaseOrdersPage_Should_HandleEscapedSubstringAndCaseVariants_OnOrderNumberSearch` in `tests/Darwin.Tests.Integration/Inventory/InventorySearchProviderNeutralIntegrationTests.cs`), Orders/shipment operator (`GetOrdersPage_Should_HandleEscapedSubstringAndCaseVariants_OnOrderNumberSearch`, `GetShipmentsPage_Should_HandleEscapedSubstringAndCaseVariants_OnTrackingNumberSearch`, `GetShipmentProviderOperationsPage_Should_HandleEscapedSubstringAndCaseVariants_OnProviderSearch` in `tests/Darwin.Tests.Integration/Orders/OrdersSearchProviderNeutralIntegrationTests.cs`), Shipping (`GetShippingMethodsPage_Should_HandleEscapedSubstringAndCaseVariants_OnCarrierSearch` in `tests/Darwin.Tests.Integration/Shipping/ShippingSearchProviderNeutralIntegrationTests.cs`), Media (`GetMediaAssetsPage_Should_HandleEscapedSubstringAndCaseVariants_OnOriginalFileNameSearch` in `tests/Darwin.Tests.Integration/Media/MediaSearchProviderNeutralIntegrationTests.cs`), Loyalty (`GetLoyaltyAccountsPage_Should_HandleEscapedSubstringAndCaseVariants_OnEmailSearch`, `GetLoyaltyRedemptionsPage_Should_HandleEscapedSubstringAndCaseVariants_OnConsumerEmailSearch` in `tests/Darwin.Tests.Integration/Loyalty/LoyaltySearchProviderNeutralIntegrationTests.cs`), and business/communication operations (`GetBusinessCommunicationSetupPage_Should_HandleEscapedSubstringAndCaseVariants_OnNameSearch`, `GetEmailDispatchAuditsPage_Should_HandleEscapedSubstringAndCaseVariants_OnRecipientEmailSearch`, `GetProviderCallbackInboxPage_Should_HandleEscapedSubstringAndCaseVariants_OnProviderSearch` in `tests/Darwin.Tests.Integration/Businesses/BusinessCommunicationSearchProviderNeutralIntegrationTests.cs`) as well as remaining high-value catalog paths (`GetVariantsPage_Should_HandleEscapedSubstringAndCaseVariants_OnSkuSearch`, `GetAddOnGroupsPage_Should_HandleEscapedSubstringAndCaseVariants_OnNameSearch` in `tests/Darwin.Tests.Integration/Catalog/CatalogSearchProviderNeutralIntegrationTests.cs`), CMS page lookup (`GetPagesPage_Should_HandleEscapedSubstringAndCaseVariants_OnSlugSearch` in `tests/Darwin.Tests.Integration/CMS/CmsSearchProviderNeutralIntegrationTests.cs`), business discovery/list (`GetBusinessesForDiscovery_Should_HandleEscapedSubstringAndCaseVariants_OnNameSearch` in `tests/Darwin.Tests.Integration/Businesses/BusinessDiscoverySearchProviderNeutralIntegrationTests.cs`), and Billing operator (`GetBillingPlansAdminPage_Should_HandleEscapedSubstringAndCaseVariants_OnCodeSearch` in `tests/Darwin.Tests.Integration/Billing/BillingSearchProviderNeutralIntegrationTests.cs`).
- [done] Add CMS admin-list coverage proving soft-deleted page translations are ignored by search and localized title projection. (`GetPagesPage_Should_Ignore_SoftDeleted_Translations_In_Title_Search`, `GetPagesPage_Should_Fallback_To_NonDeleted_Translation_When_RequestCulture_Is_SoftDeleted` in `tests/Darwin.Tests.Unit/CMS/CmsPageHandlerTests.cs`)
- [done] Add cart/add-on localization coverage proving soft-deleted add-on option/value translations are ignored in cart summaries and Admin add-on option counts ignore soft-deleted options. (`HandleAsync_Should_Ignore_SoftDeleted_AddOnValue_Translations_WhenSelectingCulture` in `tests/Darwin.Tests.Unit/CartCheckout/ComputeCartSummaryHandlerTests.cs`, `GetAddOnGroupsPage_Should_Count_Only_NonDeleted_Options` in `tests/Darwin.Tests.Unit/Catalog/CatalogQueryHandlerTests.cs`)
- [done] Add WebApi JWT settings coverage proving soft-deleted `SiteSetting` rows are ignored when signing/validation parameters are refreshed.
(`GetParameters_Should_Ignore_SoftDeleted_SiteSetting_WhenCacheHasExpired` in `tests/Darwin.WebApi.Tests/Security/JwtSecurityTests.cs`)
- [done] Add cross-application `SiteSetting` soft-delete coverage proving JWT issuing/refresh, app bootstrap, business invitation/email templates, phone verification, password reset, DHL shipment defaults, and SMS/WhatsApp transports ignore soft-deleted settings rows. (`GetParameters_Should_UseActiveSiteSetting_When_SoftDeleted_SiteSetting_Also_Exists`, `HandleAsync_Should_Ignore_SoftDeleted_Settings_When_Active_Row_Is_Present`, `CreateBusinessInvitation_Should_Ignore_SoftDeleted_Settings_WhenActive_Row_Is_Present`, `RequestPhoneVerification_Should_Ignore_SoftDeleted_SiteSetting_WhenActiveSettingsExist`, `HandleAsync_Should_Ignore_SoftDeleted_Settings_When_Active_Row_Exists`, `AddShipmentHandler_Should_Ignore_SoftDeleted_Settings_AndQueueDhlOperation_When_ActiveSettingsExist`, `ProviderBackedSmsSender_Should_Ignore_SoftDeleted_SiteSetting_WhenActive_Row_Exists`, `MetaWhatsAppSender_Should_Ignore_SoftDeleted_SiteSetting_When_Active_Row_Exists`, `RequestPasswordResetHandlerTests.HandleAsync_Should_Ignore_SoftDeleted_Settings_When_Active_Row_Exists`)
- [done] Add WebAdmin communication cooldown coverage proving soft-deleted `ChannelDispatchAudit` rows do not extend admin test-message cooldown windows.
(`HandlePageAsync_Should_Ignore_SoftDeleted_ChannelDispatchAudit_ForAdminTestCooldownWindow` in `tests/Darwin.Tests.Unit/Businesses/ChannelDispatchActivityHandlerTests.cs`)
- [done] Add business-invitation boundary coverage proving pending invitations become expired exactly at `ExpiresAtUtc` consistently across preview, list, and filters.
(`GetBusinessInvitationPreview_Should_ReturnExpired_WhenExpiresAtIsExactlyNow`, `GetBusinessInvitationPreview_Should_ReturnPending_WhenExpiresAtIsOneSecondInFuture` in `tests/Darwin.Tests.Unit/Businesses/BusinessInvitationOnboardingHandlersTests.cs`;
`GetBusinessInvitationsPage_Should_ProjectExpiredStatus_WhenExpiresAtIsExactlyNow`, `GetBusinessInvitationsPage_Should_ProjectPendingStatus_WhenExpiresAtIsOneSecondInFuture`, `GetBusinessInvitationsPage_FilterExpired_Should_IncludeInvitation_WhenExpiresAtIsExactlyNow`, `GetBusinessInvitationsPage_FilterPending_Should_ExcludeInvitation_WhenExpiresAtIsExactlyNow`, `GetBusinessInvitationsPage_FilterOpen_Should_IncludeInvitation_WhenExpiresAtIsExactlyNow` in `tests/Darwin.Tests.Unit/Businesses/BusinessCommunicationAndInvitationQueryHandlersTests.cs`).
- [done] Add provider-neutral guard coverage that fails on ad-hoc/unescaped application-query `EF.Functions.Like`, query-side `Enum.ToString()` search, or query-side `Guid.ToString()` search unless a provider-specific exception is documented.
(`SecurityAndPerformanceQueryConventionsSourceTests` in `tests/Darwin.Tests.Unit/Security`).
- [done] Add provider-neutral guard coverage that fails when EF query predicates embed moving `DateTime.UtcNow` expressions directly instead of using local UTC snapshots/cutoff parameters.
(`SecurityAndPerformanceQueryConventionsSourceTests` in `tests/Darwin.Tests.Unit/Security`).
- [done] Add model-metadata coverage proving all decimal properties have explicit precision either through entity-specific configuration or the provider-neutral fallback convention.
(`ProviderNeutralDecimalPrecisionModelTests` in `tests/Darwin.Infrastructure.Tests/Persistence`).
- [done] Add PostgreSQL provider-registration coverage proving runtime and design-time Npgsql paths preserve normalized connection defaults for application name, retry limits, timeouts, keepalive, and auto-prepare settings.
(`AddConfiguredPersistence_Should_NormalizePostgreSqlConnectionDefaults` in `tests/Darwin.Infrastructure.Tests/Persistence/ServiceCollectionExtensionsConfiguredPersistenceTests.cs`; `CreateDbContext_Should_UseNormalizedPostgreSqlDefaults_WhenConfiguredForPostgreSql` in `tests/Darwin.Infrastructure.Tests/Persistence/DesignTimeDbContextFactoryTests.cs`).
- [done] Add runtime composition smoke coverage for `Darwin.WebApi` and `Darwin.Worker` with `Persistence:Provider=PostgreSql`, including localization, clock, Data Protection, identity infrastructure, and background-worker DI validation without relying on WebAdmin-only registrations. (`AddWebApiComposition_Should_RegisterPostgreSqlCompositionCoreServices`, `AddWorkerServices_Should_ValidateBackgroundWorkerDI_WhenPostgreSqlIsConfigured` in `tests/Darwin.WebApi.Tests/Composition/RuntimeCompositionTests.cs`)
- [done] Add Data Protection configuration coverage proving WebAdmin, WebApi, and Worker share `ApplicationName=Darwin`, use the configured shared key path, and fail startup when `DataProtection:RequireKeyEncryption=true` but the configured certificate thumbprint cannot be resolved. (`AddWebApiAndWorker_Should_ShareConfiguredDataProtection_Settings_WithDarwinApplicationName`, `AddWebComposition_Should_UseConfiguredDataProtectionKeysPath_AndDefaultDarwinApplicationName`, `AddWebApiComposition_Should_FailToStart_WhenKeyEncryptionRequiredAndCertificateThumbprintCannotBeResolved`, `AddWorkerComposition_Should_FailToStart_WhenKeyEncryptionRequiredAndCertificateThumbprintCannotBeResolved`, `AddWebComposition_Should_FailToStart_WhenKeyEncryptionRequiredAndCertificateThumbprintCannotBeResolved` in `tests/Darwin.WebApi.Tests/Composition/DataProtectionCompositionTests.cs` and `tests/Darwin.WebAdmin.Tests/Security/DataProtectionCompositionTests.cs`)
- [done] Add cart-line identity coverage proving add/update/remove operations canonicalize `SelectedAddOnValueIdsJson`, so the same selected add-on GUIDs match regardless of caller JSON ordering or duplicate IDs.
- [done] Add billing provider-event correlation coverage proving `EventLogs.PropertiesJson` candidate rows are confirmed by JSON value matching and do not produce false correlations from unrelated substring matches in Stripe webhook payload keys or larger string values.
- [done] Add WebAdmin/worker queue-status coverage proving provider callback inbox and shipment provider operation successful worker completions are written as `Processed`, and that legacy `Succeeded` rows still appear in processed summaries/filters until old customer data is normalized.
- [done] Add Worker retry-timing and webhook payload-integrity coverage proving each processing iteration uses a stable UTC snapshot for retry cutoffs/attempt timestamps and webhook retries do not embed stale `PayloadHash` values into newly signed payload envelopes.
- [done] Add Worker multi-instance queue-claim coverage proving concurrent workers skip rows already claimed through optimistic concurrency before executing external side effects.
- [done] Add Worker completion-save resilience coverage proving queue completion and inactive webhook-subscription batch updates retry transient database failures and handle post-claim concurrency without crashing the worker loop.
- [done] Add notification sender idempotency coverage proving SMTP/SMS/WhatsApp create a pending audit claim before external sends, skip duplicate sends when a non-deleted active audit (`Pending` or `Sent`) already exists for the same `CorrelationKey`, allow old failed/pending retry flows with new correlation keys, and enforce one active audit through SQL Server/PostgreSQL unique filtered/partial indexes.
- [done] Add admin text override JSON coverage proving business/site-setting validators, public business text resolution, and WebAdmin admin-text localization all use the same object-of-culture-to-string-map structure, reject structurally invalid values, and do not throw on duplicate/case-variant keys.
- [done] Add Business operations RowVersion coverage proving onboarding provisioning, provider-callback inbox actions, and communication dispatch cancellation reject missing/stale row versions and compare null database row versions without raw exceptions.
 - [done] Add WebApi provider-webhook boundary coverage proving public Stripe/DHL webhook endpoints reject payloads larger than the configured/raw 256 KiB cap with HTTP 413 before signature verification or inbox persistence, and still accept valid payloads within the cap.
- [done] Add WebApi provider-webhook idempotency coverage proving concurrent Stripe/DHL callbacks with the same provider idempotency key create only one active inbox row and subsequent requests return `duplicate=true`.
- [done] Add Stripe webhook processing idempotency coverage proving concurrent processing of the same Stripe event creates only one non-deleted `EventLog` row and the losing save path returns a duplicate result without applying a second observable update.
- [done] Add WebAdmin provider-callback inbox coverage proving Stripe/DHL payload previews show bounded operational summaries instead of raw JSON, invalid legacy JSON falls back safely, and unknown-provider previews redact obvious secret/token/signature fields.
- [done] Add WebAdmin operational action coverage proving null or empty row versions return validation/concurrency failures without raw exceptions across billing management, billing plans, catalog/CMS edits, CRM edits, identity role/user-role edits, inventory edits/lifecycle, loyalty account actions, media/pricing/SEO/settings/shipping edits, provider callback inbox, shipment provider operation, communication dispatch cancellation, business onboarding provisioning, and shipment carrier exception resolution. (`billing plan`, `role/user-role`, and catalog brand/category/product/page edit paths are covered in `WebAdminSecuritySmokeTests`; CRM customer/lead/opportunity/invoice/segment edit paths are now covered in `WebAdminSecuritySmokeTests`; inventory warehouse/supplier/stock-level/stock-transfer/purchase-order paths are now covered in `WebAdminSecuritySmokeTests`; loyalty campaign-delivery status paths are now covered in `WebAdminSecuritySmokeTests`.)
- [done] Add WebAdmin operational action coverage proving action values are trimmed and case-insensitive for billing webhook delivery, payment dispute review, provider callback inbox, CRM lead/opportunity lifecycle, and shipment provider operations.
- [done] Add WebAdmin operational concurrency coverage proving billing webhook delivery, provider callback inbox, and shipment provider operation actions convert post-row-version-save concurrency conflicts to `ItemConcurrencyConflict` and manual requeue clears retry cooldown state so workers can pick the row immediately.
- [done] Add CRM RowVersion coverage proving customer, lead, opportunity, invoice edit, invoice status, invoice refund, customer segment edit, and lead/opportunity lifecycle operations reject missing/stale row versions and compare null database row versions without raw exceptions. (WebAdminSecuritySmokeTests now covers customer/lead/opportunity/invoice/segment edit and invoice status/refund row-version miss/stale validators.)
- [done] Add Inventory RowVersion coverage proving warehouse, supplier, stock-level, stock-transfer, stock-transfer lifecycle, purchase-order, and purchase-order lifecycle operations reject missing/stale row versions and compare null database row versions without raw exceptions. (`WebAdminSecuritySmokeTests` now covers the currently enumerated inventory row-version operations.)
- [done] Add CRM/Inventory admin post-save concurrency coverage proving customer, lead, lead conversion, opportunity, invoice edit/status/refund, customer segment edit, warehouse, supplier, stock-level, stock-transfer, stock-transfer lifecycle, purchase-order, and purchase-order lifecycle operations convert database concurrency exceptions after the initial row-version check into localized admin-safe conflict responses.
- [done] Add Loyalty RowVersion coverage proving account activation/suspension/adjustment, reward confirmation, scan-session expiry, program/reward-tier edit/delete, business campaign edit/activation, and campaign delivery status operations reject missing/stale row versions where required and compare null database row versions without raw exceptions.
- [done] Add dedicated WebAdmin loyalty edit row-version coverage proving `Loyalty/EditProgram` and `Loyalty/EditRewardTier` reject missing row versions and stale row versions safely.
- [done] Add Loyalty/Orders admin post-save concurrency coverage proving loyalty account activation/suspension/adjustment, reward confirmation, loyalty program/reward-tier edit/delete, order status update, and shipment carrier exception resolution operations convert database concurrency exceptions after the initial row-version check into localized admin-safe conflict responses.
- [done] Add Loyalty scan-session and storefront checkout race coverage proving concurrent scan confirmations return localized scan-session conflicts instead of raw concurrency exceptions, and concurrent cart checkout attempts produce a single order with a stable unique order number while the losing request receives a safe checkout conflict.
- [done] Add WebAdmin enum-boundary validation coverage proving out-of-range numeric enum values are rejected for business operational status, CMS page status, add-on selection mode, and promotion type instead of being persisted.
- [done] Add WebAdmin lifecycle transition coverage proving business approval only applies to pending businesses, suspension only applies to approved businesses, reactivation only applies to suspended businesses, and closed CRM opportunities cannot be advanced.
- [done] Add WebAdmin Business post-save concurrency coverage proving business lifecycle, business edit/delete, location edit/delete, media edit, member edit/delete, onboarding provisioning, and communication-dispatch cancel operations convert database concurrency exceptions after the initial row-version check into localized admin-safe conflict responses.
...
- [done] Add Business invitation WebAdmin concurrency coverage proving invitation list rows render Base64 row versions, resend/revoke reject missing or stale row versions, retry-from-email-audit uses the current invitation row version, and post-save conflicts surface localized item conflict messages before any duplicate invitation email is sent.
 - [done] Add worker webhook-delivery retry coverage proving a successful external callback is not re-sent when the normal completion save hits a concurrency conflict, because the fallback completion update persists status, response code, payload hash, attempt time, and retry count for the same delivery.
- [done] Add WebAdmin billing transition coverage proving payment status edits reject unsupported transitions such as terminal `Failed`/`Refunded`/`Voided` moving back to active states and `Completed` moving anywhere except `Refunded`.
- [done] Add Billing RowVersion coverage proving payment, financial-account, expense, journal-entry, billing-plan, webhook-delivery, dispute-review, and subscription cancel-at-period-end operations reject missing/stale row versions and compare null database row versions without raw exceptions.
- [done] Add Billing admin post-save concurrency coverage proving payment, financial-account, expense, journal-entry, billing-plan, and payment dispute review updates convert database concurrency exceptions after the initial row-version check into localized admin-safe conflict responses.
- [done] Add order billing boundary coverage proving payments cannot be created directly as `Refunded`/`Voided`, `Completed` payments set paid timestamps and can advance early orders to paid, and refunds are only allowed for `Captured` or `Completed` payments.
...
- [done] Add Orders RowVersion coverage proving order status changes, shipment carrier exception resolution, and shipment provider operation actions reject missing/stale row versions and compare null database row versions without raw exceptions.
...
- [done] Add WebAdmin pricing/settings/shipping/media/SEO post-save concurrency coverage proving Promotion, Tax Category, Site Setting, Shipping Method, Media Asset edit/delete/purge, and SEO redirect-rule update/delete operations convert database concurrency exceptions after the initial row-version check into localized admin-safe conflict responses.
- [done] Add Identity role/user assignment coverage proving role-permission and user-role updates reject missing/stale row versions and compare null database row versions without raw exceptions, alongside validator coverage for address, permission, user, and profile row-version payloads.
- [done] Add WebAdmin form-rendering coverage proving edit forms render RowVersion hidden fields as valid Base64 values instead of framework-formatted byte-array placeholders, across catalog, billing, business, CRM, inventory, media, settings, shipping, role/permission, and user-role editors.
...
- The mobile-only solution filter includes this lane so local mobile validation does not accidentally skip shared route/security/service contracts.
- CI also runs release-like Windows target builds for Shared, Consumer, and Business plus Android Debug builds for Consumer and Business. Android Release remains intentionally excluded from CI until Firebase and Google Maps production inputs are supplied securely.

### Mobile.Consumer tests

- `Darwin.Mobile.Consumer.Tests` is the Consumer app launch-readiness guard lane. It covers Android cleartext/backup hardening, Release Firebase and Google Maps configuration fail-fast contracts, Apple Release APNs entitlement shape, Consumer resource parity, absence of committed Google API keys, hidden customer-facing promotion diagnostics, removed legacy template pages, and push-registration orchestration without MAUI static singleton dependencies.
- CI runs this lane on `windows-latest` after installing the MAUI workload. It is a guard lane for source contracts and pure testable coordinators; it is not a replacement for Android/iOS/MacCatalyst release packaging or device-provider smoke.
- Run it when changing Consumer manifests, app project packaging, settings/feed/profile/account-deletion surfaces, push registration, resources, or platform release config:
...
- Latest local result on 2026-05-24: `120` total consumer tests, `0` failed, `0` skipped.
- Runtime smoke on 2026-05-24 installed the Android Debug build on the emulator and validated Consumer login, Discover, Google Maps rendering, and QR generation. This does not replace signed release packaging or device/provider smoke.

### Mobile.Business tests

- `Darwin.Mobile.Business.Tests` is the Business app launch-readiness guard lane. It covers Android cleartext/backup hardening for both mobile manifests, absence of committed Google/Firebase secret patterns, English/German resource parity, read-only subscription status plus HTTPS management-website handoff, source-contract soft-gate coverage for live operation ViewModels, WebAdmin mobile support surfaces, and unsafe certificate trust fail-fast behavior.
- CI runs this lane on `windows-latest` after installing the MAUI workload. It proves the launch guards remain present, but deeper ViewModel/UI behavior, platform packaging, and provider-backed device workflows still need dedicated release validation.
- Run it when changing Business manifests, subscription/status surfaces, scanner/session/reward/dashboard gates, WebAdmin support routes used by mobile operations, resources, or shared mobile security configuration:
...
- Latest local result on 2026-05-20: `11` passed, `0` failed, `0` skipped.
- Runtime smoke on 2026-05-24 installed the Android Debug build on the emulator and validated Business login, Home, and the Scan tab. The non-camera loyalty QR API flow also passed against `https://api.loyan.de` by preparing, processing, and confirming an accrual session. Physical camera scanning still requires a second-device/device-camera validation pass.

### Mobile build validation

- CI `mobile-build-validation` restores `Darwin.MobileOnly.slnf`, builds the three mobile projects for `net10.0-windows10.0.19041.0` in `Release`, and builds Consumer/Business for `net10.0-android` in `Debug`.
- This lane proves the current build graph remains healthy without production secrets. It is not a substitute for signed Android/iOS/MacCatalyst release artifacts, store metadata checks, device smoke, Firebase/APNS validation, or Google Maps production-key validation.
- Tizen is not part of the current launch target set. Placeholder Tizen manifests are ignored unless the product owner explicitly adds Tizen back to scope.

- [done] Consumer app broader auth/discovery/detail/rewards/QR and payment/email handoff debt reduced by adding source-contract coverage for activation/reset query aliases, non-overwriting prefill, forgot-password reset handoff, and member-commerce invoice artifact actions plus checkout-handoff wording contract.
- [done] Consumer login email-and-entry debt reduced by adding source-contract coverage for `LoginPage` action bindings/activation handoff and `LoginViewModel` readiness + activation-email request/visibility contracts.
- [done] Consumer loyalty commanding debt reduced by adding source-contract coverage for `RewardsViewModel` quick-QR command, business-selection guard, navigation payload, and empty-business precondition contract, plus `SettingsPage` command binding contract.
- [done] Consumer authentication/account lifecycle and email-contact surfaces debt reduced by adding source-contract coverage for `ChangePasswordViewModel/Page`, `LegalHubViewModel/Page`, and `MemberPreferencesViewModel/Page` contracts (feedback, busy/command gating, account-deletion navigation guard, and marketing consent gating with row-version persistence).
- [done] Consumer profile and navigation-surface debt reduced by adding source-contract coverage for `ProfilePage` (command bindings + atomic guarded navigations), `ProfileViewModel` lifecycle and command setup, and profile refresh/save/push/verification behavior.
- [done] Consumer address-book debt reduced by adding source-contract coverage for `MemberAddressesPage` and `MemberAddressesViewModel` contracts (refresh/edit/save/delete/default commands, row-version handling on update path, lifecycle wiring).
- [done] Consumer loyalty feed debt reduced by adding source-contract coverage for `FeedPage` and `FeedViewModel` (refresh/pagination/promotion/diagnostics contracts, loyalty navigation context handoff, and busy-state command gating).
- [done] Consumer payment action-deck debt reduced by adding source-contract coverage for `MemberCommercePage` order-detail/track-and-tracking action bindings and `MemberCommerceViewModel` order-command initialization contracts.
- [done] Consumer commerce runtime command-gating debt reduced by adding `MemberCommerceCommandGatingTests` checks for `ViewOrderCommand`, `ViewInvoiceCommand`, and `OpenOrderShipmentTrackingCommand` (null/nonnull command parameter and tracking-link gating).
- [done] Consumer payment-email runtime command-gating debt reduced by adding busy-state gating checks for `MemberCommerceViewModel` payment/artifact and `ActivationViewModel` email commands, plus activation command initialization source-contract coverage.
- [done] Consumer payment/email runtime behavior debt reduced by adding activation request/confirm execution checks (input validation + trimmed-email + service delegation + failure messaging and success messaging), and `MemberCommerceViewModel` order/invoice payment intent execution checks that verify `HostedCheckout` delegation and error messaging on failed payment intent creation.
- [done] Consumer payment/email runtime behavior debt further reduced by asserting `ActivationViewModel.OnDisappearingAsync` is safe when no cancellable operation is active.
- [done] Consumer payment/email runtime behavior debt further reduced by adding `ActivationViewModel` confirm-success/confirm-failure execution checks and `MemberCommerceViewModel` invoice-detail + invoice/order document copy execution checks (guarding on availability and verifying failed-response messaging).
- [done] Consumer payment/runtime guard debt further reduced by adding commerce runtime guard/error checks for load/detail methods, retry eligibility, and invoice copy/export preconditions before service delegation.
- [done] Consumer payment/runtime guard debt further reduced by asserting retry-payment handoff failures when checkout URLs are missing for both order and invoice flows before checkout launch.
- [done] Consumer payment/runtime guard debt further reduced by adding missing precondition tests for order/invoice document-copy methods when no selected runtime context exists.
- [done] Consumer payment/runtime guard debt further reduced by adding shipment-tracking runtime guards for null shipment and missing/blank tracking URLs.
- [done] Consumer payment/runtime guard debt further reduced by asserting refresh error surfacing for failed order and invoice list refresh calls.
- [done] Consumer commerce runtime debt further reduced by asserting `OnAppearingAsync` single-refresh behavior and safe `OnDisappearingAsync` execution when no operation is active.
- [done] Consumer payment/runtime exception-path debt further reduced by adding exception-path coverage for activation request/confirm calls and `MemberCommerceViewModel` order/invoice refresh/detail/payment-retry/document-copy paths when service methods throw.
...
- [done] Add integration coverage for `Email:Provider=Brevo` using Brevo sandbox mode (`X-Sib-Sandbox=drop`) so the API contract can be validated without sending real emails (`EmailDispatchOperationBackgroundServiceTests` now verifies sandbox header behavior via `BrevoEmailSender` payload request assertions).
- [done] Add configuration/DI coverage proving `IEmailSender` resolves to Brevo or SMTP based on `Email:Provider`, fails fast for unsupported providers, and fails startup when Brevo is active without required Brevo options.
- [done] Add Worker coverage for queued `EmailDispatchOperation.Provider=Brevo` and legacy `SMTP` rows, verifying status transitions and `ProviderMessageId` capture.
- [done] Add WebApi coverage for the Brevo webhook endpoint: Basic Auth required, oversized payload rejected, invalid payload rejected, duplicate event idempotency, and valid event persisted into `ProviderCallbackInboxMessages`.
  - [done] Completed in this branch: credential whitespace handling, oversized/invalid payload paths, event normalization (case/whitespace/truncation), and duplicate detection for trimmed message-id/idempotency keys.
  - [done] Additional coverage added in this pass: whitespace-only credential values fail configuration validation; oversize detection also validated for UTF-8 multi-byte payload bodies; mixed-case JSON field names are accepted (`EvEnT`, `MeSsAgE-iD`, `Ts`, `DATE`).
- [done] Add WebApi coverage for the DHL webhook endpoint: missing/invalid site settings and signature validation.
  - [done] Completed in this pass: `DhlApiSecret` whitespace treated as not configured, oversized payload detection for UTF-8 multi-byte bodies, and payload that deserializes to `null` returns invalid payload.
- [done] Add WebApi coverage for the Stripe webhook endpoint: configured secret validation and malformed envelope robustness.
  - [done] Completed in this pass: whitespace `StripeWebhookSecret` is treated as missing, and JSON arrays are rejected as invalid payloads.
- [done] Add WebApi service coverage for `StripeWebhookSignatureVerifier` parser edge-cases.
  - [done] Completed in this pass: signature header entries can be reordered (`v1` before `t`) while still validating correctly.
- [done] Add WebApi service coverage for ProviderCallbackInboxWriter null/empty idempotency semantics.
...
- [done] Add Worker coverage for Brevo webhook processing against `EmailDispatchAudit`.
  - [done] Completed in this pass: delivered/open/click keep successful audit state, hard/soft bounce/spam/blocked/invalid/error mark the audit failed with provider reason (including reason trimming and default missing reason), unsupported events do not overwrite successful state, failed audits do not regress to Sent on delivery events, soft-deleted audits are not matched, and correlation keys are matched through mixed-case alias fields.
