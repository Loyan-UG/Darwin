...
- ✅ Add external-command smoke evidence for legal-validation failure propagation to protect against non-compliant artifacts even when process exit code is zero:
...
- ✅ Add Brevo production-readiness smoke evidence for sandbox send, controlled inbox send, transactional webhook subscription, callback inbox persistence, and worker processing. (`BrevoReadinessSmoke_Should_BlockWhenDeliveryPipelinePrerequisitesAreMissing` and `BrevoReadinessSmoke_Should_ReportDeliveryPipelineReadinessInDryRun` added to `SecurityAndPerformanceContractsAndPackagingSourceTests`.)
- ✅ Structured invoice model mapping and download flow follow-through is now implemented after tooling decision:
...
- ✅ Add provider-selection smoke coverage proving `Persistence:Provider=PostgreSql` resolves Npgsql and `Persistence:Provider=SqlServer` resolves SQL Server. (`ServiceCollectionExtensionsConfiguredPersistenceTests` in `tests/Darwin.Infrastructure.Tests/Persistence`)
- ✅ Add PostgreSQL migration/seed verification against a fresh PostgreSQL database. (`FreshPostgreSqlDatabase_Should_ApplyMigrations_AndSeedExpectedRows` in `tests/Darwin.Tests.Integration/Persistence`)
...
- ✅ Add a guard that shared EF mappings do not introduce SQL Server-only column types such as `uniqueidentifier` or `nvarchar(max)` in shared persistence mappings.  
...
- ✅ Add PostgreSQL `jsonb` coverage proving the current 21 selected operational/configuration JSON columns are created as `jsonb`, and the current 14 `IX_PG_*_JsonbGin` indexes exist.  
(`PostgreSqlProviderSpecificColumnTypeTests` and `PostgreSqlSearchIndexCoverageTests` in `tests/Darwin.Infrastructure.Tests/Persistence`).
