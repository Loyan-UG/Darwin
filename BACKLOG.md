...
- `Stripe`: test-mode WebApi handoff smoke now creates a real Stripe-hosted Checkout Session and confirms the storefront return route does not mark payments successful. Remaining Stripe blocker: pay through Stripe test checkout and confirm verified webhook events finalize payment/order state, then later repeat the smoke with live keys before production traffic.
- `DHL`: run live smoke against the target DHL account. Confirm account-specific base URL, authentication, product codes, shipment payload, label PDF retrieval/storage, tracking reference persistence, callback processing, and WebAdmin recovery for failed/stuck provider operations. A guarded local prerequisite/validation harness exists at `scripts/smoke-dhl-live.ps1`; real smoke remains blocked until approved account settings are configured.
- `Brevo`: complete production DNS and account readiness: sender domain verification, DKIM/DMARC, sandbox off, Basic Auth webhook configured, `ProviderCallbackWorker` and `EmailDispatchOperationWorker` enabled. A guarded sandbox/controlled-inbox harness exists at `scripts/smoke-brevo-readiness.ps1`; real production readiness still requires account DNS and webhook verification.
...
- `Invoice archive/object storage`: keep the internal/database archive provider as the development fallback. The code now has a named invoice archive provider router, a file-system provider, a reusable `ObjectStorage` abstraction/options boundary, and an SDK-backed S3-compatible adapter for MinIO/AWS-style storage. Production direction is MinIO through the S3-compatible provider, with AWS S3 and Azure Blob as alternatives. Validate MinIO Object Lock/versioning with target credentials before claiming production archive immutability.
- `E-invoice`: structured invoice source-model JSON and a provider-neutral `IEInvoiceGenerationService` boundary now exist, but the default generator is explicitly `NotConfigured`; real ZUGFeRD/Factur-X generation and validation are still required before claiming full e-invoice compliance. XRechnung remains a secondary export backlog item; use `docs/e-invoice-tooling-decision.md` for the tooling decision checklist.
...
- Complete Stripe test-mode hosted checkout and webhook verification after the local handoff smoke: pay the generated Stripe test Checkout Session, confirm `checkout.session.completed` and `payment_intent.succeeded` processing, and verify failed/refund/dispute visibility where applicable.
- Run DHL live smoke after target account credentials and product settings are entered; start with `scripts/smoke-dhl-live.ps1` prerequisite checks and only use `-Execute` for an approved real validation request.
...
- Run `scripts/smoke-brevo-readiness.ps1 -Execute -Sandbox`, then one controlled-inbox `-Execute` pass after Brevo DNS and sender approval are complete.
- Run S3-compatible/MinIO external smoke for save/read/metadata/presigned URL/object lock behavior, then keep the invoice archive policy wrapper selected only where retention/legal-hold requirements pass. Keep Azure Blob as an explicit supported alternative provider boundary until its SDK adapter is implemented.
- Implement the selected e-invoice generator behind `IEInvoiceGenerationService` and wire it through the existing `GenerateInvoiceEInvoiceArtifactHandler`: extend the current structured invoice source-model mapping into the selected ZUGFeRD/Factur-X model, validate output, expose downloadable compliant artifacts, and add tests. Add XRechnung export later.
...
- External immutable invoice archive provider hardening: MinIO/S3-compatible external smoke, bucket Object Lock/versioning validation, and Azure Blob adapter later if selected by deployment.
