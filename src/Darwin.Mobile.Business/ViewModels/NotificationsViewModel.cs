...
public sealed class NotificationsViewModel : BaseViewModel
{
    ...
    private async Task RefreshAsync()
    {
        ...
        try
        {
            ...
            var response = await _notificationInboxService
                .GetAsync(NotificationInboxTargetApp.Business, SelectedCategory?.Category, 1, 80, cts.Token)
            ...
            var items = response.Value.Items.Select(NotificationDisplayItem.FromContract).ToList();
            var unreadResult = await _notificationInboxService
                .GetUnreadCountAsync(NotificationInboxTargetApp.Business, cts.Token)
                .ConfigureAwait(false);
    ...
    private async Task MarkReadAsync(NotificationDisplayItem? item)
    {
        ...
        var result = await _notificationInboxService
            .MarkReadAsync(item.Id, NotificationInboxTargetApp.Business, CancellationToken.None)
            .ConfigureAwait(false);
        if (!result.Succeeded)
        {
    ...
    {
        var result = await _notificationInboxService
            .MarkAllReadAsync(NotificationInboxTargetApp.Business, CancellationToken.None)
            .ConfigureAwait(false);
        if (!result.Succeeded)
        {
            ErrorMessage = AppResources.NotificationsLoadFailed;
    ...
    [
        ...
        new(NotificationInboxCategory.Billing, AppResources.NotificationsCategoryBilling),
        new(NotificationInboxCategory.ScannerSession, AppResources.NotificationsCategoryScannerSession),
