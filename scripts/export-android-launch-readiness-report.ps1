param(
    [string]$OutputPath = "artifacts\production-readiness\android-launch-readiness-report.md",
    [switch]$Force
)
...
function Test-ContainsSensitivePattern {
    ...
    $blockedPatterns = @(
        ...
        "(?i)\b(keystore|signing[_ -]?key|service[_ -]?account|google-services\.json)\s*[:=]\s*\S+",
        "(?i)\b(raw provider payload|provider payload|raw payload|device log)\s*[:=]\s*\{"
    )
...
try {
    $rawOutput = & powershell -NoProfile -ExecutionPolicy Bypass -File "scripts\check-android-launch-readiness.ps1" 2>&1
...
if (Test-ContainsSensitivePattern $outputText) {
    throw "Android readiness output appears to contain a secret assignment, private key, signing key, keystore value, service-account value, provider payload, or device log. Refusing to write report."
}
...
$lines.Add("# Android Launch Readiness Report")
$lines.Add("")
...
$lines.Add("")
$lines.Add("This report is a non-secret readiness summary generated from `scripts\check-android-launch-readiness.ps1`. It does not store signing keys, keystore paths, Firebase service-account credentials, API keys, OAuth secrets, private package artifacts, provider payloads, device logs, customer data, or private approval records.")
$lines.Add("")
...
if (Test-ContainsSensitivePattern $report) {
    throw "Generated Android readiness report appears to contain a secret assignment, private key, signing key, keystore value, service-account value, provider payload, or device log. Refusing to write report."
}
...
Write-Host "Android launch readiness report created:"
