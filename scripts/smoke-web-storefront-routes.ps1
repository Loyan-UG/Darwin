...
function Assert-SafeUrl {
    ...
    if (-not [string]::IsNullOrWhiteSpace($uri.Query) -or
        ...
        Write-Host "Web storefront route smoke is blocked."
        Write-Host "$Name must be a base deployment URL without query strings or fragments. Keep API keys, auth tokens, route state, and provider payloads out of route smoke input."
        exit 2
    }
