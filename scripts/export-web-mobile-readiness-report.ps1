...
$checks = @(
    ...
    @{
        Name = "Android project readiness prerequisites"
        Command = @("powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts\check-android-project-readiness.ps1")
    }
)
...
$lines.Add("")
$lines.Add("This report is a non-secret readiness summary for Web storefront and mobile resource/project prerequisites. It runs the local storefront production build only when dependencies are already present from the lockfile, checks whether route-smoke configuration is present, and validates Android project metadata without reading secret mobile configuration files. It does not run npm install, build mobile packages, execute external provider calls by default, store npm tokens, registry credentials, environment files, API keys, auth cookies, private package artifacts, customer data, provider payloads, or device logs.")
$lines.Add("")
$lines.Add("## Summary")
$lines.Add("")
...
$lines.Add("")
$lines.Add("Use this report as a non-secret attachment reference for production-like staging and Android-first launch evidence. A `Blocked` result is expected until the deployment owner provides approved storefront runtime, executed route smoke, degraded API log review, staging owner sign-off, and mobile launch evidence. A Ready local build row proves only that the checked-out storefront builds locally; a Ready route-smoke prerequisite row proves only that the smoke can be executed against the configured public routes; a Ready Android project row proves only that checked-in Android project metadata and guards are locally coherent.")
