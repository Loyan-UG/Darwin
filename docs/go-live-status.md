...
- A local production-like evidence working copy and non-secret readiness report bundle were generated on 2026-06-18 under `artifacts\production-readiness\`, which is ignored by git. The report bundle covers production-like staging, local backup package structure, local PostgreSQL restore rehearsal, local .NET release-candidate build/test lanes, Web/Mobile readiness, aggregate go-live dry-run, MinIO, Azure Blob, e-invoice, Android launch, and provider readiness, then `scripts\check-production-readiness-report-bundle.ps1` validates the local report set shape. It does not clear external deployment blockers.
