...
- Consumer mobile launch hardening advanced on 2026-05-20 and was re-verified on 2026-05-24: Android cleartext remains disabled, Android app backup is disabled for `Darwin.Mobile.Consumer`, customer-facing promotion diagnostics were removed from the Feed page, legacy template pages were removed, settings navigation moved into the ViewModel, stale selected order/invoice details are cleared after commerce refresh failures, and the dedicated `Darwin.Mobile.Consumer.Tests` lane passed with `120` passed, `0` skipped.
- Business mobile release hardening now also disables Android platform backup, and `Darwin.Mobile.Business.Tests` guards the Business manifest, shared mobile package parity, Tizen out-of-scope status, resource parity, read-only subscription handoff, source-contract soft-gates for live operation screens, WebAdmin support surfaces, inactive outbox launch scope, safe mobile shell navigation logging, and unsafe certificate trust behavior. The Business guard lane passed locally on 2026-05-20 with `11` passed and `0` skipped.
- Android emulator runtime smoke on 2026-05-24 validated Consumer login, Discover, Google Maps rendering, Consumer QR generation, Business login, Business Home, and the Business Scan tab without XAML/runtime crashes. The loyalty QR non-camera API path also passed through `prepare -> process -> confirm accrual` against `https://api.loyan.de`. Camera-based QR scanning from a second device/emulator remains a manual/device smoke item.
...
Status: webhook boundary, internal reconciliation, and real Stripe Checkout Session creation path are implemented; local/mock checkout fallback has been removed from payment-intent response paths; test-mode storefront hosted checkout, return-route guard, signed webhook delivery, ProviderCallbackWorker processing, business subscription checkout finalization, provider-backed refund reconciliation, and dispute follow-up processing pass locally/staging through `https://api.loyan.de`.
...
- WebAdmin Site Settings preserves existing secret values when secret fields are posted blank or with the masked placeholder. This prevents normal settings edits from clearing Stripe, DHL, SMTP, JWT, SMS, or WhatsApp secrets unless an explicit non-placeholder value is supplied.
...
- Stripe test-mode handoff smoke passed locally on 2026-05-10 against a rebuilt WebApi instance from the current source on `http://localhost:5136`: the script created a disposable public checkout smoke order, received a Stripe-hosted Checkout Session from `checkout.stripe.com`, and confirmed the storefront return route left the payment `Pending`.
- Stripe staging smoke passed again on 2026-05-24 through `https://api.loyan.de`: the script created a disposable public checkout order, received a Stripe-hosted Checkout Session, and confirmed the storefront return route left the payment `Pending`. A hosted Stripe test-card payment then completed the Checkout Session as `paid`, Stripe delivered signed `payment_intent.succeeded` and `checkout.session.completed` events to Darwin, and `ProviderCallbackWorker` processed both inbox rows. The matching Darwin payment moved from `Pending` to captured/provider-finalized state with `PaidAtUtc` set. The run also confirmed the webhook public URL preflight, created and paid a business subscription Checkout Session, processed signed `checkout.session.completed` and `invoice.paid` subscription events, and updated the matching `BusinessSubscription` with provider checkout, customer, subscription references, and `StartedAtUtc`. WebAdmin `Orders/AddRefund` created a provider-backed Stripe refund, Stripe returned provider status `succeeded`, a signed `charge.refunded` event reached Darwin, and `ProviderCallbackWorker` processed it. The Stripe endpoint was then subscribed to `charge.dispute.created`, `charge.dispute.updated`, `charge.dispute.funds_withdrawn`, and `charge.dispute.closed`; a real test-mode `charge.dispute.created` webhook reached Darwin and marked the matched payment as failed with an `UnderReview` dispute marker for operator follow-up. The subscription success redirect reached `admin.loyan.de`, which returned Cloudflare `502` because WebAdmin was not running during that browser step; webhook finalization still completed.
- Local execution note: run WebApi from `src/Darwin.WebApi` or through the launch profile so `appsettings.Development.json` is loaded. Starting the compiled WebApi DLL from the repository root omits `StorefrontCheckout:FrontOfficeBaseUrl` and correctly blocks payment-intent creation.
...
- Stripe test-mode storefront hosted checkout, verified webhook finalization, subscription checkout finalization, provider-backed refund reconciliation, and dispute follow-up processing are now confirmed at the documented public endpoint. Before production traffic, repeat the smoke with live keys and verify operational monitoring/alerting against real provider events.
- The subscription reconciliation workspace exists. Stripe live-mode smoke remains pending.
...
- Focused internal Brevo/communication verification passed on 2026-05-24: `Darwin.WebApi.Tests` Brevo/email-dispatch/provider-callback lane passed with `308` passed and `0` skipped, and the related unit communication/provider-callback lane passed with `232` passed and `0` skipped. This verifies internal worker, webhook, dispatch, row-version, and guarded smoke-script behavior; it is not a production DNS/delivery validation.
...
- Go-live readiness dry-run behavior coverage now executes `scripts/check-go-live-readiness.ps1` from the unit suite and verifies the readiness summary, provider prerequisite sections, accepted ready/blocked status, provider-ready dry-run status with fake non-secret prerequisites, selected archive/e-invoice decisions, and secret-output guard. The focused tests `GoLiveReadinessScript_Should_RunDryRunAndAvoidSecretOutput` and `GoLiveReadinessScript_Should_ReportProviderReadyAndSelectedDecisionsReady` passed locally on 2026-05-13 with `2` passed and `0` skipped.
...
- E-invoice phase 1 now has a structured invoice source-model export from issued snapshots, a minimum source-readiness validator, JSON/XML source-model downloads, a provider-neutral `IEInvoiceGenerationService` boundary, an optional disabled-by-default external command adapter for deployment-approved generator tooling, an external-command smoke harness, and an `IEInvoiceArtifactStorage` boundary that can persist generated artifacts through the invoice archive object-storage profile when a real generator is configured. The external command adapter rejects empty, over-sized, non-PDF ZUGFeRD/Factur-X, and malformed XRechnung XML outputs before storage, but this is not a substitute for legal format validation. The selected first tooling path is Mustangproject CLI through the external-command adapter, but no compliant e-invoice artifact is generated until a pinned wrapper is configured, deterministic fixtures pass, and deployment smoke succeeds. The primary target remains ZUGFeRD/Factur-X because SME users need a human-readable PDF plus structured data. XRechnung remains a secondary export backlog item. Current JSON/HTML/CSV/source-model exports are not full e-invoice compliance.
- E-invoice selected tooling path and retained alternatives are documented in `docs/e-invoice-tooling-decision.md`. The optional smoke command is `powershell -NoProfile -ExecutionPolicy Bypass -File scripts\smoke-einvoice-external-command.ps1 -Execute` after `DARWIN_EINVOICE_COMMAND_PATH` points to an approved wrapper.
...
- Mobile launch-readiness hardening now disables app-wide Android cleartext traffic for both MAUI apps, disables Android platform backup for both MAUI apps, removes the local hardcoded Google Maps key file from the workspace, keeps mobile API access on HTTPS, aligns shared mobile package versions between Consumer and Business, avoids logging raw navigation exception details from mobile shells, and adds source-contract guards for cleartext traffic, committed mobile Google API keys, unsafe certificate trust, English/German resource key parity, package drift, Business read-only subscription handoff, Business soft-gate coverage, WebAdmin mobile support surfaces, and inactive outbox launch scope. `Darwin.Mobile.Consumer` can now request member invoice archive documents and structured JSON/XML source-model exports through canonical member invoice routes, but these exports remain operational artifacts and are not compliant e-invoices.
- `Darwin.Mobile.Business` subscription management remains intentionally read-only inside the app: operators can refresh status and open the configured HTTPS management website. Full in-app plan checkout/cancel management remains a product decision, not completed launch scope.
- Mobile status is conditional rather than fully launch-ready: the implemented Consumer and Business workflows are operationally usable and covered by shared, Consumer, and Business guard lanes, and CI now also runs release-like Windows target builds plus Android Debug build validation. Store launch still requires production mobile configuration, signed Android/iOS/MacCatalyst package validation, provider/device smoke, and resolution of the remaining Consumer checkout and Business subscription-scope decisions. Tizen is out-of-scope for the current launch target set.
