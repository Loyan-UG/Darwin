...
Latest DHL provider-callback worker smoke result: `Darwin.Worker` processed a temporary DHL `shipment.delivered` inbox row with only `ProviderCallbackWorker` enabled. The matching shipment moved to `Delivered`, tracking metadata, label URL, `LastCarrierEventKey`, and one `Shipping.ShipmentCarrierEvents` row were written, the inbox row moved to `Processed`, and all temporary smoke rows were removed after validation.
...
Development Data Protection note: Worker and web entry points use shared Data Protection registration so identity/secret services can resolve during startup. Current local development settings point WebAdmin, WebApi, and Worker at `%LOCALAPPDATA%\Darwin\DataProtectionKeys` with `DataProtection:ApplicationName=Darwin`; the shared registration expands environment variables before creating the directory so cookies/tokens protected by shared infrastructure remain readable across entry points that intentionally share keys without hardcoding a developer machine path.
