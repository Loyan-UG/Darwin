...
- Build the ZUGFeRD/Factur-X generation service behind the existing `IEInvoiceGenerationService` boundary, including artifact validation, storage through the invoice archive profile, and documentation for any selected generator/tooling. Keep optional XRechnung export as a secondary item unless a deployment explicitly requires it earlier.
