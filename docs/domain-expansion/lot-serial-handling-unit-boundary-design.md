...
- `ProductTrackingPolicy`, `InventoryLot`, `InventorySerialUnit`, `HandlingUnit`, and `HandlingUnitContent` now exist as the canonical internal Inventory traceability identities with WebAdmin setup and review surfaces.
- Receipt identity capture, receipt inline identity creation, and transfer/count/pick identity integration are complete for current internal/WebAdmin flows. Recall, expiry attention, exact-unit stock derivation, and handling-unit labels still require dedicated integration slices before those workflows enforce or print additional identity evidence.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Product tracking policy | `ProductTrackingPolicy` exists for product variants and is enforced by current receipt, transfer, count, and picking handlers. | Use explicit tracking policy before enforcing lot/serial capture. | Catalog plus Inventory policy services. | Product/Inventory handlers. | Determines when identity is required for receipt, transfer, pick, count, and return. | None directly. | PWA can show required scan fields later. | Enforcement complete for current receipt, transfer, count, and pick flows. | Derive bin and tracked-identity availability from movement evidence. |
| Lot identity | `InventoryLot` exists with supplier lot, expiry/manufacture dates, status, and safe metadata. | Lot identity remains structured quantity identity. | Inventory identity services. | Lot handlers and receipt/import owners. | Enables expiry, quarantine, recall, and lot-level availability. | Supplier invoice matching remains quantity/cost based. | Scan/display later. | Core complete; movement linkage pending. | Capture on goods receipt. |
| Serial identity | `InventorySerialUnit` exists as unique unit identity linked to product variant and optional lot. | Serial identity remains structured exact-unit identity. | Inventory identity services. | Serial handlers and receipt/import owners. | Enables exact-unit movement, warranty, recall, and shipment traceability. | None directly. | PWA scan required for serial-tracked products. | Core complete; movement linkage pending. | Capture on goods receipt. |
...
| Handling unit identity | `HandlingUnit` exists with barcode, status, type, optional warehouse/location, and parent relation. | Handling unit is grouped warehouse execution identity with contents. | Warehouse identity services. | Handling unit handlers. | Supports pallet/carton/tote movement without faking locations. | None directly. | PWA scan candidate. | Core complete; movement linkage pending. | Capture on goods receipt and movement workflows. |
| Handling unit contents | `HandlingUnitContent` exists with product quantity and optional lot/serial identity links. | Contents must reference product quantity and optional lot/serial identity. | Warehouse identity services. | Handling unit content handlers. | Prevents grouped stock ambiguity. | None. | PWA can display contents. | Core complete; current receipt/transfer/count/pick enforcement is implemented. | Use content evidence in bin/handling-unit availability derivation. |
| Receipt capture | `GoodsReceiptLine` records accepted product variant quantity and `GoodsReceiptLineIdentity` captures tracked identity evidence. | Tracked products require identity evidence before posting. | Goods receipt and identity handlers. | Goods receipt lifecycle plus identity capture handlers. | Stock increase remains receipt-owned and ledger-backed. | Supplier invoice matching still uses posted receipt evidence. | PWA receiving scan later. | Complete for current internal/WebAdmin flow. | Use receipt evidence in bin/identity stock derivation. |
| Transfer capture | `StockTransferLine` moves product variant quantity and `StockTransferLineIdentity` preserves tracked evidence. | Transfer must preserve required identity evidence for tracked products. | Stock transfer handlers. | Transfer lifecycle handlers. | Prevents identity loss between warehouses/bins. | None. | PWA transfer scan later. | Complete for current internal/WebAdmin flow. | Use transfer evidence in bin/identity stock derivation. |
| Picking capture | `WarehouseTaskLine` captures picked/short product quantity and `WarehouseTaskLineIdentity` preserves tracked evidence. | Picking tracked products must validate and preserve lot/serial/handling unit evidence. | Warehouse task handlers plus allocation owners. | Picking handlers. | Prevents wrong-lot or wrong-serial fulfillment. | None. | PWA pick scan later. | Complete for current internal/WebAdmin flow. | Use picking evidence in bin/identity stock derivation and later PWA scan UX. |
| Stock count capture | `StockCountLine` captures expected/count/variance by variant and `StockCountLineIdentity` preserves tracked evidence. | Tracked-product counts require structured lot/serial/handling unit evidence. | Stock count handlers. | Count lifecycle handlers. | Variance can be identity-aware without a second ledger. | None directly. | PWA count scan later. | Complete for current internal/WebAdmin flow. | Use count evidence in bin/identity stock derivation. |
| Bin-level stock | `WarehouseLocation` exists and internal/WebAdmin `BinStock` derives bin and tracked-identity availability from stock levels plus task/count/identity evidence. | Bin-level stock is derived from ledger-adjacent evidence and must not become a parallel authoritative quantity. | Inventory movement projections. | Projection/query handlers. | Preserves single movement authority and surfaces unassigned or inconsistent evidence as attention. | None. | PWA can read derived stock. | Complete for current WebAdmin/internal phase. | Use derived availability in PWA execution screens. |
...
1. `Lot Serial And Handling Unit Core Model And Admin Slice`
   - Complete. `ProductTrackingPolicy`, `InventoryLot`, `InventorySerialUnit`, `HandlingUnit`, and `HandlingUnitContent` are implemented in Inventory with WebAdmin internal setup/review, PostgreSQL/SQL Server migrations, safe metadata validation, event/audit evidence, and source guards.
...
2. `Receipt Identity Capture Slice`
   - Complete for link-existing receipt evidence. `GoodsReceiptLineIdentity` records preserve lot, supplier lot, expiry, serial, handling-unit, quantity, and safe metadata snapshots under `GoodsReceiptLine`.
   - Goods receipt posting now requires structured lot, serial, expiry, and handling-unit evidence when active `ProductTrackingPolicy` requires it.
   - WebAdmin inspection can link existing lot, serial unit, and handling unit records without creating stock movement or a second stock ledger.
3. `Receipt Inline Identity Creation Slice`
   - Complete. Goods receipt inspection can create missing lot, serial unit, and handling-unit records in place and link them immediately to receipt identity evidence.
   - Inline creation remains internal/WebAdmin-only and does not post stock or bypass receipt posting validation.
4. `Transfer Count Pick Identity Integration Slice`
   ...
   - Transfer dispatch/receive, stock count approval/posting, and picking completion enforce active product tracking policies.
   - `InventoryTransaction` remains the stock movement ledger.
5. `Bin-Level Stock Derivation Slice`
   ...
   - No parallel authoritative stock ledger is added.
6. `Recall And Expiry Operations Slice`
