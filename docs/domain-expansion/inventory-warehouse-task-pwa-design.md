...
- Stock count boundaries are locked in [stock-count-boundary-design.md](stock-count-boundary-design.md), and the internal/WebAdmin stock count core is implemented. Lot/serial and handling unit boundaries are locked in [lot-serial-handling-unit-boundary-design.md](lot-serial-handling-unit-boundary-design.md). There is no lot/serial core model, handling unit core model, pick/pack workflow, printer-specific adapter, or warehouse PWA contract yet.
...
| Stock level summary | `StockLevel` exists per warehouse/product. | Keep as summary; do not turn it into the detailed ledger or lot/bin owner. | Inventory services. | Existing stock level and stock movement handlers. | Summary remains fast read model for availability. | None. | PWA can display available/reserved summaries. | Ready. | Add bin/lot summary only after detailed movement owner is defined. |
...
| Stock counts | `StockCountSession` and `StockCountLine` exist with WebAdmin list/detail/create/update/lifecycle workflow. | Formal count documents own count evidence and reviewed variances. Manual adjustment alone is not a count workflow. | Inventory count services. | Stock count handlers. | Count variance posts through inventory adjustment owner after review. | Finance valuation impact remains separate future accounting design. | PWA supports scan/count entry and review later. | Complete for internal/WebAdmin v1. | Design lot/serial and handling units before exact-unit counts. |
| Lot and serial tracking | Boundary design is complete; no canonical core model exists yet. | Add lot/serial tracking only as structured inventory identity, not custom fields, when product policy requires it. | Inventory identity services. | Lot/serial handlers plus receipt/transfer/pick/count posting handlers. | Enables regulated traceability, expiry, recall, and exact-unit stock. | Supplier invoice/payment remains unaffected. | PWA scan workflows must support lot/serial capture when required. | Ready for core implementation. | Implement product tracking policy and structured identity model. |
| Handling units | Boundary design is complete; no canonical core model exists yet. | Add handling units as grouped warehouse execution identity; do not fake them as bins or lots. | Warehouse handling-unit services. | Handling unit handlers. | Supports grouped movement and putaway/pick efficiency. | None directly. | PWA can scan handling unit labels. | Ready for core implementation. | Implement handling unit and content records with lot/serial links. |
...
12. `Lot Serial And Handling Unit Core Model And Admin Slice`
   - Implement product tracking policy, lot identity, serial unit identity, handling unit identity, and handling unit content records.
   - Implement structured identity records and WebAdmin internal review without changing public/mobile/storefront or finance/supplier/customer flows.
13. `Warehouse Mobile-First PWA Slice`
   - Implement internal/operator PWA routes after task foundation and online-first mutation policy are stable.
   - Keep public/member mobile contracts unchanged.
...
- Future PWA routes are internal/operator routes, not public storefront routes and not Consumer/Business mobile DTO changes.
