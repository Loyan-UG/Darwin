...
This document locks the Purchasing expansion boundary and records the completed Purchasing implementation slices through supplier contacts, supplier document metadata, and formal goods receipt. The design step was documentation-only; subsequent implementation slices added schema, Application, and WebAdmin changes for the existing `Supplier`, `SupplierContact`, `PurchaseOrder`, `GoodsReceipt`, and related line models only. They did not add public routes, mobile contracts, storefront changes, supplier invoice shortcuts, invoice archive/download changes, or production finance flow changes.
...
- `SupplierContact` exists in the Inventory area for structured supplier contacts with role, name, email, phone, language, primary-contact flag, row-version, and archive behavior.
...
- Warehouse location/bin, task, stock count, lot/serial, handling unit, and mobile-first PWA boundaries are locked in [inventory-warehouse-task-pwa-design.md](inventory-warehouse-task-pwa-design.md). Goods receipt remains the official receiving document; warehouse tasks execute work and do not replace receipt ownership.
...
| Supplier contacts | `SupplierContact` exists as an Inventory child of `Supplier`. | Structured frequent contacts are real records; do not overload supplier notes or JSON for purchasing, AP, logistics, quality, or technical contacts. | Purchasing supplier services. | `Activity`, `Note`, custom fields for customer-specific contact metadata. | Additive supplier contact table implemented. No public/mobile contract change. | None directly. | Contact can support invoice and payment communication later. | Unchanged. | P1 | Keep contact ownership on supplier master; do not duplicate contacts in invoices, payments, or warehouse tasks. |
...
| Warehouse task execution | Warehouse task design is locked separately; no canonical task model exists yet. | Warehouse tasks can execute receiving, putaway, transfer, picking, and count work later, but they do not replace `GoodsReceipt`, `StockTransfer`, order allocation, or `InventoryTransaction`. | Future warehouse task services. | `BusinessEvent`, `AuditTrail`, `DocumentRecord`, `ExternalReference`. | No change in this document. | Task posting delegates to existing stock owners. | Supplier invoice/payment remains finance-owned. | Unchanged. | P0 | Implement warehouse location/bin core before task execution. |
...
| Purchasing documents and attachments | `DocumentRecord` exists and supplier document metadata is visible in WebAdmin supplier editing. | Supplier contracts, purchase order documents, delivery notes from suppliers, receipt photos, and invoice attachments use `DocumentRecord` metadata. | Foundation document service. | `DocumentRecord`. | Metadata registration implemented for suppliers only. No upload/download route or payment shortcut. | Receipt documents can support stock acceptance. | Supplier invoice evidence uses documents later. | Unchanged. | P1 | Keep binary upload/download and external exposure behind dedicated document design. |
...
- Supplier contacts are structured when frequent/reportable; do not hide common contact records in notes or JSON.
...
- Warehouse tasks execute warehouse work after their own foundation exists; they do not replace goods receipts, stock transfers, order allocation, or inventory transaction ownership.
...
3. `Purchasing Documents And Supplier Contacts Slice`
   ...
   - Outcome: completed. `SupplierContact` is implemented in the Inventory schema with WebAdmin create, update, archive, row-version, primary-contact behavior, duplicate email guard, and safe metadata checks. Supplier document metadata is registered through `DocumentRecord` and shown in the supplier editor without upload/download, supplier invoice/payment shortcuts, public/mobile exposure, or finance export format changes.
