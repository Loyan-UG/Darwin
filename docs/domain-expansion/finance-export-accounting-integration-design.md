...
| Conflict handling | `SyncConflict` is not implemented. | V1 export is outbound-only. Conflicts from target systems are not modeled until inbound sync or two-way reconciliation is designed. | Future sync/conflict foundation. | Future integration service. | No conflict ids in v1 export. | Avoids false bidirectional sync promises. | Unchanged. | Unchanged. | Blocked until sync design. | Keep export one-way unless two-way requirements are explicit. |
