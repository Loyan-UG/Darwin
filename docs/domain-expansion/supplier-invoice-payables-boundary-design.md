...
| Supplier payment boundary | Payment/refund flows exist for customer/order settlement; supplier payment is not canonical. | Supplier payment settlement is a separate future flow and must not be faked as refund, order payment, or manual journal-only state. | Future supplier payment service. | Future payables payment handlers. | Payment reduces payable only through finance posting and settlement linkage. | Export requires posted supplier payment entries. | Unchanged. | Not part of core invoice model. | Design supplier payment boundary before implementation. |
...
- Supplier payment settlement is a separate future flow and is not implemented by customer payment, refund, negative receivable, or journal shortcut behavior.
...
3. `Supplier Payment Boundary Design Slice`
   - Design supplier payment settlement, partial payment, payment export, and payable aging before implementation.
   - Do not reuse customer payment/refund flows as supplier settlement.
4. `Supplier Documents And Contacts Slice`
...
The next step should design supplier payment settlement, partial payment, payment evidence, payable aging, export impact, and reversal rules before implementation. It must not reuse customer payment/refund flows as supplier settlement and must keep public/mobile/storefront exposure out of scope unless explicitly designed.
