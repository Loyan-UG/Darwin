...
- The Stripe Dashboard endpoint must subscribe to `charge.dispute.created`, `charge.dispute.updated`, `charge.dispute.funds_withdrawn`, and `charge.dispute.closed` so dispute follow-up queues receive provider signals.
...
Staging note from 2026-05-24: with WebApi running from `src/Darwin.WebApi`, `DARWIN_WEBAPI_BASE_URL=https://api.loyan.de`, and `ProviderCallbackWorker:Enabled=true`, the public checkout smoke created a disposable order, returned a Stripe-hosted Checkout Session, confirmed the return route did not complete the payment, completed hosted Stripe test-card payment, received signed `payment_intent.succeeded` and `checkout.session.completed` events, and processed both callback inbox rows. The matching Darwin payment moved to captured/provider-finalized state with `PaidAtUtc` set. The business subscription checkout smoke also returned a Stripe-hosted Checkout Session, completed hosted test-card payment, processed signed `checkout.session.completed` and `invoice.paid` events, and updated the matching `BusinessSubscription` with provider checkout, customer, subscription references, and `StartedAtUtc`. WebAdmin `Orders/AddRefund` then created a provider-backed Stripe refund, stored provider status `succeeded`, received signed `charge.refunded`, and processed it through `ProviderCallbackWorker`. The Stripe endpoint was then extended with dispute events; a real test-mode `charge.dispute.created` webhook reached Darwin and `ProviderCallbackWorker` marked the matched payment for dispute follow-up. The subscription success redirect reached `admin.loyan.de`, which returned Cloudflare `502` because WebAdmin was not running during that browser step; webhook finalization still completed. Starting the compiled WebApi DLL from the repository root is not equivalent to the launch profile because it does not load `src/Darwin.WebApi/appsettings.Development.json`.

Latest local test-mode handoff result: the handoff smoke passed on 2026-05-10 against a rebuilt WebApi instance from the current source on `http://localhost:5136`. The script created a disposable public checkout smoke order, received a Stripe-hosted Checkout Session from `checkout.stripe.com`, and confirmed the return route left the payment `Pending`. The focused WebApi Stripe webhook/provider-callback suite also passed on 2026-05-10 with `191` passed and `0` skipped using an isolated output path. The later `https://api.loyan.de` staging run completed storefront hosted-checkout, subscription webhook finalization, provider-backed refund reconciliation, and test-mode dispute follow-up processing; live-mode smoke still needs to be completed before production traffic.
